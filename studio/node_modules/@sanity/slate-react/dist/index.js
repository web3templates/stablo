    ...
    if (newDomRange) {
      ...
      if (selection) {
        try {
          ...
          if (block && slate.Editor.isVoid(editor, block) && boundary) {
            var element = ReactEditor.toDOMNode(editor, block);
            var boundingRect = element.getBoundingClientRect();
            voidOffset = boundingRect.height; // Get container properties

            var cTop = boundary.scrollTop;
            var cBottom = cTop + boundary.clientHeight; // Get element properties
...
var ReactEditor = {
  ...
  toDOMNode: function toDOMNode(editor, node) {
    var KEY_TO_ELEMENT = EDITOR_TO_KEY_TO_ELEMENT.get(editor);
    var domNode = slate.Editor.isEditor(node) ? EDITOR_TO_ELEMENT.get(editor) : KEY_TO_ELEMENT && KEY_TO_ELEMENT.get(ReactEditor.findKey(editor, node));
...
var withReact = function withReact(editor) {
  var e = editor;
  var apply = e.apply,
      onChange = e.onChange; // The WeakMap which maps a key to a specific HTMLElement must be scoped to the editor instance to
