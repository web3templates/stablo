...
async function importDocuments(documents, options) {
  ...
  documents.some(documentHasErrors.validate); // Validate that there are no duplicate IDs in the documents

  ensureUniqueIds(documents); // Replace relative asset paths if one is defined
  ...
  const batches = batchDocuments(weakened); // Ensure that we don't reference missing assets, or assets in different datasets

  debug('Validating asset documents');
  await validateAssetDocuments(docs, options); // Trigger actual import process

  debug('Starting import of documents');
  const docsImported = await importBatches(batches, options); // Documents are imported, now proceed with post-import operations

  debug('Uploading assets');
  const {
    failures: assetWarnings
  } = await uploadAssets(assetRefs, options); // Strengthen references

  debug('Strengthening references');
  await strengthenReferences(strongRefs, options); // Return number of documents imported
