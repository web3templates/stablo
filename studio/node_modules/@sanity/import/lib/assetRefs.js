...
const assetMatcher = /^(file|image)@([a-z]+:\/\/.*)/; // Note: mutates in-place

function unsetAssetRefs(doc) {
  findAssetRefs(doc).forEach(path => {
    const parentPath = path.slice(0, -1);
    const parent = get(doc, parentPath); // If the only key in the object is `_sanityAsset`, unset the whole thing,
...
function absolutifyPaths(doc, absPath) {
  ...
  const modifier = value => value.replace(/file:\/\/\.\//i, `${getFileUrl(absPath, {
    resolve: false
...
function getAssetRefs(doc) {
  return findAssetRefs(doc).map(path => validateAssetImportKey(path, doc)).map(path => ({
    documentId: doc._id,
    path: serializePath({
      path: path.filter(isNotAssetKey)
    }),
...
function validateAssetImportKey(path, doc) {
  if (!assetMatcher.test(get(doc, path))) {
    throw new Error(['Asset type is not specified.', '`_sanityAsset` values must be prefixed with a type, eg image@url or file@url.', `See document with ID "${doc._id}", path: ${serializePath({
      path
    })}`].join('\n'));
  }
