...
const requireContext = (directory, recursive, regExp) => {
  ...
  try {
    ...
    const resolveFrom = require(resolvePath); // We need to resolve `./foo` from where the _caller_ is situated
    ...
    const keys = klaw(basePath, {
      depthLimit: recursive ? 30 : 0,
      nodir: false
    }) // Make sure we only match the provided regexp (or use the default (.json/.js))
    .filter(file => file.path.match(regExp || /\.(json|js)$/)) // Use relative paths for the keys
...
function register() {
  ...
  try {
    module.constructor.wrap = script => {
      const requireContextScript = requireContext.toString().replace(/hookPath/g, JSON.stringify(hookPath)).replace(/klawPath/g, JSON.stringify(klawPath)).replace(/resolvePath/g, JSON.stringify(resolvePath)); // Assigning the function may very well crash in an upcoming version,
  ...
  registered = true; // There isn't a good way to clean this up
